...
function getCustomerBalance($id) {
    ...
    function getRemainingBySupplier($id){
        ...
        $query =  $this->db->select('journals.customer as customerName, items.name ,sum(quantity) as totalquantity')
        ->join('items','items.id=stock.item_name','left')->join('journals','stock.journal_id = journals.id','left')
